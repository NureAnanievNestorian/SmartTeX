...
class DiffSafetyReviewService(SmallModelCallMixin):
    ...
    def _deterministic_review_payload(review_input, stats) -> dict[str, Any]:
        return {
            ...
            "added_text_samples": review_input.added_text_samples,
            "content_loss_signals": review_input.content_loss_signals,
            "semantic_summary": review_input.semantic_summary,
        ...
        if total_changed > max_changed:
            deterministic_warnings.append(
                warning(
                    ...
                    human_title="Зміна більша, ніж очікувалось",
                    human_detail="Перевір, чи всі змінені рядки справді потрібні для цього запиту.",
                )
            )
            if effective_advisory:
                deterministic_warnings.append(
                    warning(
                        ...
                        "scope_confidence",
                        human_title="Ліміт зміни спрацював як підказка",
                        human_detail="Розмір patch може бути нормальним, але його варто швидко переглянути перед прийняттям.",
                    )
                )
        if files_changed > max_files:
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Зачеплено більше файлів, ніж планувалося",
                    human_detail=f"Зміна торкнулася {files_changed} файлів при очікуваному ліміті {max_files}.",
                )
            )
        if hunks > max_hunks:
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Зміни розкидані по документу",
                    human_detail="Кілька окремих hunk-ів можуть означати, що AI зачепив зайві місця.",
                )
            )
        if review_input.deleted_headings:
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Видалено заголовок",
                    human_detail=f"Зник заголовок: {', '.join(review_input.deleted_headings[:3])}. Якщо це не було явним завданням, proposal ризикований.",
                    locations=review_input.deleted_headings[:3],
                )
            )
        if review_input.deleted_text_samples and stats.get("lines_removed", 0) > stats.get("lines_added", 0) + 2:
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Можлива втрата тексту",
                    human_detail="AI видалив більше змістовного тексту, ніж додав. Перевір, чи не зник абзац або теза поза запитом.",
                    samples=review_input.deleted_text_samples[:3],
                )
            )
        for signal in review_input.content_loss_signals:
            kind = str(signal.get("kind") or "")
            if kind in {"deleted_heading", "deleted_refs", "changed_imports"}:
                continue
            deterministic_warnings.append(
                warning(
                    str(signal.get("severity") or "medium"),
                    "CONTENT_LOSS_TRAP",
                    str(signal.get("detail") or "Possible accidental content loss detected."),
                    "content_loss_trap",
                    human_title=str(signal.get("title") or "Можлива втрата контенту"),
                    human_detail=str(signal.get("detail") or ""),
                    samples=signal.get("removed_samples") or [],
                    token_overlap=signal.get("token_overlap"),
                )
            )
        if review_input.deleted_labels_or_refs:
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Видалено посилання або цитування",
                    human_detail="Можуть зламатися cross-reference, bibliography або Typst/LaTeX labels.",
                )
            )
        if review_input.changed_imports_or_includes:
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Змінено підключення файлів",
                    human_detail="Imports/includes/bibliography впливають на весь документ, навіть якщо diff виглядає малим.",
                )
            )
        ...
        if (
            ...
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Patch вийшов за заявлений scope",
                    human_detail="Це не обовʼязково помилка, але перед accept варто перевірити кожен файл і hunk.",
                )
            )
        ...
        if (
            ...
            deterministic_warnings.append(
                warning(
                    ...
                    "deterministic_diff_stats",
                    human_title="Diff завеликий для прямої перевірки",
                    human_detail="Потрібен ручний перегляд: мала модель бачить лише стислий зріз великого diff.",
                )
            )
        ...
        if result.get("recommendation") == "warn_user":
            warnings.append(
                warning(
                    ...
                    human_title="Safety reviewer бачить ризик",
                    human_detail="Мала модель знайшла ризик у запропонованій зміні. Перевір diff перед прийняттям.",
                )
            )
        if result.get("suspicious_deletions"):
            warnings.append(
                warning(
                    ...
                    "diff_safety_reviewer",
                    human_title="Підозріле видалення",
                    human_detail="Safety reviewer вважає, що частина видаленого тексту може бути не повʼязана із запитом.",
                    samples=result.get("suspicious_deletions") or [],
                )
            )
            ...
            if not self._provider_reject_has_substantive_basis(result):
                warnings.append(
                    warning(
                        ...
                        "diff_safety_reviewer",
                        human_title="Запит на вужчий patch понижено до warning",
                        human_detail="Reviewer не знайшов конкретного semantic-ризику, тому proposal не заблоковано, але його варто переглянути.",
                    )
                )
